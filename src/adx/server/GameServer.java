...
public class GameServer extends GameServerAbstract {
  ...
  private synchronized void sendEndOfDayMessage() {
    ...
    for (Entry<String, Connection> agent : this.namesToConnections.entrySet()) {
      ...
      try {
        agent.getValue()
            .sendTCP(new EndOfDayMessage(this.serverState.getCurrentDay() + 1, timeEndOfDay.toString(), this.serverState.getDailySummaryStatistic(agentName),
                listOfCampaigns, this.serverState.getWonCampaigns(agentName), this.serverState.getQualitScore(agentName),
                this.serverState.getProfit(agentName)));
      } catch (AdXException e) {
